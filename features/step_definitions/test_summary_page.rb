Given('I navigate to the homepage (summary)') do
  visit 'https://www.gov.uk/calculate-your-holiday-entitlement'
  sleep 1
end

And('I should see the homepage (summary)') do
...
When ("I click on the 'Start now' button (summary)") do
  click_link('Start now')
end

And("I select the option no for working irregular hours (summary)") do
...
And ('I select the option hours worked per week (summary)') do
...
And ('I select the option for a full leave year (summary)') do
...
And ('I input 37.5 hours worked per week (summary)') do
...
And ('I input 5 days worked per week (summary)') do
...
Then ('I should see the correct submitted answers (summary)') do
...
And ('I should see the total entitlement hours (summary)') do
