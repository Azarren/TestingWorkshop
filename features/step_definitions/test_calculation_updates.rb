Given('I navigate to the homepage (updates)') do
  visit 'https://www.gov.uk/calculate-your-holiday-entitlement'
  sleep 1
end

And('I should see the homepage (updates)') do
...
When ("I click on the 'Start now' button (updates)") do
  click_link('Start now')
end

And("I select the option no for working irregular hours (updates)") do
...
And ('I select the option hours worked per week (updates)') do
...
And ('I select the option for a full leave year (updates)') do
...
And ('I input 37.5 hours worked per week (updates)') do
...
And ('I input 5 days worked per week (updates)') do
...
Then ('I should see the correct submitted answers (updates)') do
...
And ('I should see the total entitlement hours (updates)') do
